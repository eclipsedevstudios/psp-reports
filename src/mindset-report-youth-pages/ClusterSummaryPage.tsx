...
const ClusterSummaryPage = ({
  ...
  const interpretation = percentileInterpretationYouth.filter(obj => obj.name === clusterName && obj.language === language && percentile >= obj.minPercentile && percentile <= obj.maxPercentile)[0];
        ...
        <ClusterAnalysis>
          ...
          <p>
            {language === Language.Spanish ?
              ...
              :
              <>
                Your {clusterLabel} score was in the {classifyPercentile(percentile, language)} range ({percentile}th percentile) when compared to other youth athletes. {comparePercentile(percentile, clusterLabel, language)}
              </>
            }
          </p>
          <p>
