...
const FourClustersPage = ({ language }: { language: Language }) => {
  const growthMindset = clusters.filter(cluster => cluster.name === YouthCluster.GrowthMindset && cluster.language === language)[0];
  const mentalSkills = clusters.filter(cluster => cluster.name === YouthCluster.SelfConfidence && cluster.language === language)[0];
  const teamSupport = clusters.filter(cluster => cluster.name === YouthCluster.TeamCulture && cluster.language === language)[0];
  const healthHabits = clusters.filter(cluster => cluster.name === YouthCluster.HealthBehaviors && cluster.language === language)[0];
