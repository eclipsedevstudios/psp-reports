...
const FiveClustersPage = ({ language }: { language: Language }) => {
  const growthMindset = clusters.filter(cluster => cluster.name === AdultCluster.GrowthMindset && cluster.language === language)[0];
  const mentalSkills = clusters.filter(cluster => cluster.name === AdultCluster.MentalSkills && cluster.language === language)[0];
  const teamSupport = clusters.filter(cluster => cluster.name === AdultCluster.TeamSupport && cluster.language === language)[0];
  const healthHabits = clusters.filter(cluster => cluster.name === AdultCluster.HealthHabits && cluster.language === language)[0];
  const selfReflection = clusters.filter(cluster => cluster.name === AdultCluster.SelfReflection && cluster.language === language)[0];
...
const introParagraphStrings: { [key in Language]: ReactNode } = {
  [Language.English]: <>
    ...
    performance and mental wellness in athletes. All clusters have been identified as "very" or
  ...
  [Language.Spanish]: <>
    ...
    contribuyen al rendimiento óptimo y el bienestar mental de los atletas. Nuestro grupo de expertos en la
